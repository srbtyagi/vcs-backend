...
app.get('/vcsapi/api/generate/excel/employees/:user_id/:name', function (req, res) {
    // if (verifys == "verify") {
    async function apps() {
        try {
            ...
            for (i in arr) {
                var u_name = ""
                var sop_name = ""
                if (arr[i].user_middle_name === null || arr[i].user_middle_name === "") {
                    u_name = arr[i].user_first_name + " " + arr[i].user_last_name
                } else {
                    u_name = arr[i].user_first_name + " " + arr[i].user_middle_name + " " + arr[i].user_last_name
                }
                if (arr[i].supervisor_first_name !== null && arr[i].supervisor_last_name !== null) {
                    if (arr[i].supervisor_middle_name === null || arr[i].supervisor_middle_name === "") {
                        sop_name = arr[i].supervisor_first_name + " " + arr[i].supervisor_last_name
                    } else {
                        sop_name = arr[i].supervisor_first_name + " " + arr[i].supervisor_middle_name + " " + arr[i].supervisor_last_name
                    }
                }
                data =
                    data +
...
function generateExcelEmployeeData(data, data2) {
    return new Promise(function (resolve, reject) {
        ...
        wb.write(`/home/ubuntu/vcs/excle_file/employee${data.created_by}.xlsx`, function (err) {
            if (err) resolve("err");
            else resolve(`/home/ubuntu/vcs/excle_file/employee${data.created_by}.xlsx`);
...
app.get('/vcsapi/api/generate/excel/incentive_process/:employee/:f_date/:t_date/:user_id/:name', function (req, res) {
    // if (verifys == "verify") {
    async function apps() {
        try {
            let un = await getAllUser(req.params.user_id);
            ...
            var data1 = {
                created_by: user_name,
                from_month: moment(new Date(req.params.f_date)).format("MMMM") + ", " + moment(new Date(req.params.f_date)).format("YYYY"),
                to_month: moment(new Date(req.params.t_date)).format("MMMM") + ", " + moment(new Date(req.params.f_date)).format("YYYY")
            }
            var arr = []
            ...
            // res.json(arr);
            for (i in arr) {
                if (arr[i].designation === "admin" || arr[i].designation === "Manager") {
                ...
                if (arr[i].rm_name === null) {
                    ...
                    rec_name = arr[i].rf_name + " " + arr[i].rm_name + " " + arr[i].rl_name
                }
                let emp_nae = ""
                if (arr[i].em_name === null) {
                    emp_nae = arr[i].ef_name + " " + arr[i].el_name
                } else {
                    emp_nae = arr[i].ef_name + " " + arr[i].em_name + " " + arr[i].el_name
                }
                data =
                    data +
                    emp_nae +
...
function generateExcelIncentivePData(data, data2) {
    return new Promise(function (resolve, reject) {
        ...
        wb.write(`/home/ubuntu/vcs/excle_file/income_recon${data.created_by}.xlsx`, function (err) {
            if (err) resolve("err");
            else resolve(`/home/ubuntu/vcs/excle_file/income_recon${data.created_by}.xlsx`);
...
app.get("/vcsapi/get/api/tbl/conf_document/:recruitee_id", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get("/vcsapi/get/api/tbl/conf_document/:conf_doc_id/:name", function (req, res) {
    // if (verifys == "verify") {
    async function apps() {
        try {
...
app.post("/vcsapi/insert/api/tbl/conf_document/:conf_doc_name/:user_id/:recruitee_id", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
function insertConfDocument(data1, data) {
    return new Promise(function (resolve, reject) {
        ...
        file.mv("/home/ubuntu/vcs/Uploads/ConfDocument/" + filename, function (err) {
            if (err) {
                reject(err);
            } else {
                ...
                let post = {
                    ...
                    upload_datetime: moment(new Date(strTime)).format("MM/DD/YYYY h:mm:ss A"),
...
app.get("/vcsapi/get/api/tbl/system_name", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/vcsapi/api/get/action_id/role_id', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get('/vcsapi/api/get_userrole/fields', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/vcsapi/api/get/data/role_id', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                ...
                for (let i = 0; i <= get.length - 1; i++) {
                    if (indexof.length == 0) {
                        ...
                        if (indvalue == -1) {
                            ...
                            newData[indvalue].action_name = action_names + "," + get[i].action_name
                        }
                    }
                }
...
app.post('/vcsapi/api/check_post_role', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
//get user access
app.get("/vcsapi/get/api/user/access", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get("/vcsapi/get/api/checkIfexists/:job_no", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/api/api/get/action_id_by_user_id/user_id', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.put('/vcsapi/api/update/status/userrole/:role_id', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.put('/vcsapi/api/update/userrole/:role_id', stuff.verifyToken, stuff.verify, function (req, res) {
...
app.post('/vcsapi/api/post/insert/role_id', stuff.verifyToken, stuff.verify, function (req, res) {
                ...
                for (let i = 0; i < getEmployees.length; i++) {
                    ...
                    if (deleteUserAcc === "success") {
                        for (let j = 0; j < req.body.data.length; j++) {
                            let insertUserAcc = await insertUserAccess(getEmployees[i].user_id, req.body.data[j].action_id);
                        }
                    }
                }
...
app.post("/vcsapi/update/api/client_name", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/edit/api/change_status/client", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/add/api/client/client_name", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get("/vcsapi/get/all/clients", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/get/applicant/by/clientID", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get("/vcsapi/get/applicant/by/clientID/ALL/data", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/get/asign_manager/and/applicant/details", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                //console.log(req.body);
                ...
                if (req.body.manager_id !== "ALL") {
                    if (str === "") {
                        ...
                        str = str + ` and (am.recruiter_id=${req.body.manager_id} OR am.onb_mgr_id=${req.body.manager_id} OR am.team_lead_id=${req.body.manager_id} OR am.manager_id=${req.body.manager_id})`;
                    }
...
app.get("/vcsapi/get/clients", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get("/vcsapi/get/Recruiter/all", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/update/fillup/status/done", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get('/vcsapi/api/get/standard/document/data', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/vcsapi/api/update/expirY_dates/document', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/vcsapi/upload/document/:rec_doc_name/:user_id/:doc_id/:expiryDate', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                ...
                let getRdoc = await checkRecDocumentsbyrbName(rid[0].recruitee_id, req.params.rec_doc_name);

                // //console.log("getRdoc",getRdoc);
                if (rid.length) {

                    let post = await uploadFile(req.params.doc_id, req.files, rid[0].recruitee_id, sDocName, req.params.rec_doc_name, getRdoc, req.params.expiryDate);
                    if (post === "success") {
                        let getlatestrd = await getLatestRecDocuments(rid[0].recruitee_id, req.params.rec_doc_name);
...
app.post('/vcsapi/submit/document/details/:rec_doc_id/:rec_doc_name/:user_id/:doc_id', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                // //console.log(req.files)
                ...
                let rid = await getRecruitee(req.params.user_id);
                if (rid.length) {
                    let post = await updateRecrDoc(req.params.doc_id, rid[0].recruitee_id, sDocName, req.params.rec_doc_name, req.params.rec_doc_id);
                    if (post === "success") {
...
app.get('/vcsapi/get/uploaded/document/list/current/:user', function (req, res) {

    async function apps() {
        try {
            let rid = await getRecruitee(req.params.user);
            if (rid.length) {
                let checkrecrDoc = await checkRecDocumentsCurrent(rid[0].recruitee_id);
...
app.get('/vcsapi/download/:rec_doc_id/:user/:name', function (req, res) {
    ...
    async function apps() {
        try {
            let rid = await getRecruitee(req.params.user);
            if (rid.length) {
                let checkrecrDoc = await checkRecDocumentsbyrbID(rid[0].recruitee_id, req.params.rec_doc_id);
                // res.json(checkrecrDoc)
...
app.get('/vcsapi/api/get/applicants/details/:uid', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/edit/api/employee", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                let count = 0;
                ...
                if (adduadetail === "success") {
                    ...
                    if (addradetail === "success") {
                        ...
                        if (deleteUserAcc === 'success') {
                            ...
                            if (get.length) {
                                for (let i = 0; i < get.length; i++) {
                                    let insertUserAcc = await insertUserAccess(req.body.user_id, get[i].action_id);
...
app.post("/vcsapi/insert/req/doc", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                let count = 0;
...
app.post("/vcsapi/edit/api/change_status/applicant/recruit_status", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/edit/api/change_status/applicant/apply_status", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/update/req/doc/status", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/edit/api/change_password", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                ...
                let rpasswd = await updateResetPassword(req.body.user_id, hashedPassword);
                if (rpasswd === "success") {
                    ...
                    if (ud.length > 0) {
                        ...
                        let sendemail = await sendCodebyEmail(result, ud[0].user_first_name, ud[0].email, "Password")
...
app.post("/vcsapi/update/api/new/password/not/autogenerated", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                ...
                if (rpasswd === "success") {
                    ...
                    if (ud.length > 0) {
                        // //console.log("before email",result)
                        let sendemail = await sendCodebyEmail(req.body.password, ud[0].user_first_name, ud[0].email, "Password")
                        if (sendemail === "success") {
...
app.post("/vcsapi/update/api/new/passcode/not/autogenerated", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                ...
                if (rpasswd === "success") {
                    ...
                    let ud = await getAllUser(req.body.user_id);
                    if (ud.length > 0) {
                        let sendemail = await sendCodebyEmail(req.body.passcode, ud[0].user_first_name, ud[0].email, "Passcode")
                        if (sendemail === "success") {
                            res.json("success");
                        } else {
                            res.json("email not sent")
...
app.post("/vcsapi/check/api/new/password", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                ...
                // //console.log(ud[0].user_first_name);
                if (ud.length > 0) {
                    bcryptjs.compare(req.body.password, ud[0].password, function (err, result) {
                        if (result == true) {
...
app.post("/vcsapi/check/api/new/passcode", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/edit/api/change_passcode", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                ...
                if (rpasswd === "success") {
                    ...
                    let ud = await getAllUser(req.body.user_id);
                    if (ud.length > 0) {
                        let sendemail = await sendCodebyEmail(pcode, ud[0].user_first_name, ud[0].email, "Passcode")
                        if (sendemail === "success") {
                            res.json("success");
                        } else {
                            res.json("email not sent")
...
app.post("/vcsapi/edit/api/change_passcode/email", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/insert/job/post", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get("/vcsapi/get/required/doc/:user", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/edit/api/change/job_status/byjobID", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/edit/api/change/job_status/delete/byjobID", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                let job_ids = []
...
app.post("/vcsapi/edit/api/change/job_status/delete/byjobPostDate", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/edit/api/delete/skillset/byDate", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                let skillset_ids = await getSkillsetIdByDate(req.body.skillset_delete_date);
                console.log(skillset_ids)
                if (skillset_ids.length !== 0) {
                    for (i in skillset_ids) {
                        var deletestatus = await deleteMultipleskillset(skillset_ids[i].skillset_id);
                    }
...
app.post("/vcsapi/edit/api/edit/desc/n/req_info", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/vcsapi/post/signup_emp_app', function (req, res) {
    async function apps() {
        ...
        if (checkemailexist.length && checkemailexist[0].user_app_id) {
            ...
            let post = await insertUserApp(req.body, email.toLowerCase(), hashedPasscode);
            //console.log(post);
            if (post === "success") {
...
app.post('/vcsapi/post/login_employee', function (req, res) {
    async function apps() {
        ...
        if (getemail.length && getemail[0].user_app_email) {
            bcryptjs.compare("" + req.body.passcode + "", getemail[0].user_app_passcode, function (err, result) {
                if (result == true) {
                        ...
                        if (err) {
                            ...
                            if (val.length && val[0].user_id) {
                                ...
                                if (getemail[0].user_id === 0) {
                                        ...
                                        if (err1) {
                                            ...
                                            db.query(sql2, function (err2, val2) {
                                                if (err2) {
                                                    ...
                                                    res.json({ message: "login success", status: "active", user_details: val2[0] });
                                                }
                                            });
                                    ...
                                    res.json({ message: "login success", status: "active", user_details: getemail[0] });
                                }
                                ...
                                res.json({ message: "login success", status: "inactive", user_details: getemail[0] });
                            }
                        }
                    });
