...
app.get('/vcsapi/api/generate/excel/pay-rate-change-log-data/:user_id/:name', function (req, res) {
    // if (verifys == "verify") {
    async function apps() {
        try {
            ...
            for (i in p) {
                ...
                if (p[i].cb_mname === null || p[i].cb_mname === '') {
                    ...
                    changed_byName = p[i].cb_fname + " " + p[i].cb_mname + " " + p[i].cb_lname
                }

                if (p[i].proposed_start_date === null || p[i].proposed_start_date === '') {
                    pstart_date = '-'
                } else {
                    pstart_date = p[i].proposed_start_date
                }

                if (p[i].proposed_end_date === null || p[i].proposed_end_date === '') {
                    pend_date = '-'
                } else {
                    pend_date = p[i].proposed_end_date
                }

                if (p[i].onb_regular_bill_rate === null || p[i].onb_regular_bill_rate === '') {
                ...
                if (p[i].onb_holiday_bill_rate === null || p[i].onb_holiday_bill_rate === '') {
                    onb_holiday_billrate = '-'
                } else {
                    onb_holiday_billrate = p[i].onb_holiday_bill_rate
                }

                if (p[i].onb_regular_pay_rate === null || p[i].onb_regular_pay_rate === '') {
                ...
                if (p[i].onb_holiday_pay_rate === null || p[i].onb_holiday_pay_rate === '') {
                    onb_holiday_payrate = '-'
                } else {
                ...
                if (p[i].ot_starts_after_wk === null || p[i].ot_starts_after_wk === '') {
                    ot_starts_afterwk = '-'
                } else {
                ...
                if (p[i].pay_package_remarks === null || p[i].pay_package_remarks === '') {
                    pay_packageremarks = '-'
                } else {
                ...
                if (p[i].contract_duration_wk === null || p[i].contract_duration_wk === '') {
                    contract_durationwk = '-'
                } else {
...
function generateExcelPayRateChangeLogDetails(data, data2) {
    return new Promise(function (resolve, reject) {
        ...
        ws.cell(6, 3)
            ...
            .style(style1);
        ws.cell(6, 4)
            .string("Proposed End Date")
            .style(style1);
        ws.cell(6, 5)
            .string("ONB Regular BillRate")
            .style(style1);
        ws.cell(6, 6)
            .string("ONB OT BillRate")
            .style(style1);
        ws.cell(6, 7)
            .string("ONB Holiday BillRate")
            .style(style1);
        ws.cell(6, 8)
            .string("ONB Regular PayRate")
            .style(style1);
        ws.cell(6, 9)
            .string("ONB OT PayRate")
            .style(style1);
        ws.cell(6, 10)
            .string("ONB Holiday PayRate")
            .style(style1);
        ws.cell(6, 11)
            .string("Per Diem Week")
            .style(style1);
        ws.cell(6, 12)
            .string("OT Starts After")
            .style(style1);
        ws.cell(6, 13)
            .string("Total Shift Hrs")
            .style(style1);
        ws.cell(6, 14)
            .string("Shift Details")
            .style(style1);
        ws.cell(6, 15)
            .string("RTO")
            .style(style1);
        ws.cell(6, 16)
            .string("Contract Duration Week")
            .style(style1);
        ws.cell(6, 17)
            .string("Due Date")
            .style(style1);
        ...
        wb.write(`/home/ubuntu/vcs/excle_file/payRateChange{data.created_by}.xlsx`, function (err) {
            if (err) resolve("err");
            else resolve(`/home/ubuntu/vcs/excle_file/payRateChange{data.created_by}.xlsx`);
...
app.post("/vcsapi/add/api/tbl/designation", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/update/api/designation_status", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post("/vcsapi/update/api/designation_name", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get("/vcsapi/get/api/tbl/designation/dropdown/list", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get("/vcsapi/get/api/tbl/designation/all", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get('/vcsapi/api/generate/excel/candidate/:user_id/:name', function (req, res) {
    // if (verifys == "verify") {
    async function apps() {
        try {
            let un = await getAllUser(req.params.user_id);
...
function generateExcelCandidateData(data, data2) {
    return new Promise(function (resolve, reject) {
        ...
        wb.write(`/home/ubuntu/vcs/excle_file/candidate${data.created_by}.xlsx`, function (err) {
            if (err) resolve("err");
            else resolve(`/home/ubuntu/vcs/excle_file/candidate${data.created_by}.xlsx`);
...
app.get("/vcsapi/api/generate/excel/payrollProcess/:user_id/:name", function (req, res) {
...
function generateExcelPayrollProcessData(data, data2) {
    return new Promise(function (resolve, reject) {
        ...
        wb.write(`/home/ubuntu/vcs/excle_file/jobPosts${data.created_by}.xlsx`, function (err) {
            if (err) resolve("err");
            else resolve(`/home/ubuntu/vcs/excle_file/jobPosts${data.created_by}.xlsx`);
...
app.get("/vcsapi/api/generate/excel/manage/jobPost/:user_id/:name", function (req, res) {
    ...
    async function apps() {
        try {
            ...
            for (i in get_jobPost) {
                ...
                var jpe_date = ''
                var jp_date = ''
                if (get_jobPost[i].job_post_by_first_name !== null && get_jobPost[i].job_post_by_last_name !== null) {
                    if (get_jobPost[i].job_post_by_middle_name === null || get_jobPost[i].job_post_by_middle_name === '') {
                        jpb = get_jobPost[i].job_post_by_first_name + " " + get_jobPost[i].job_post_by_last_name
                    } else {
                        jpb = get_jobPost[i].job_post_by_first_name + " " + get_jobPost[i].job_post_by_middle_name + " " + get_jobPost[i].job_post_by_last_name
                    }
                }
                if (get_jobPost[i].job_post_edit_by_first_name !== null && get_jobPost[i].job_post_edit_by_last_name !== null) {
                    if (get_jobPost[i].job_post_edit_by_middle_name === null || get_jobPost[i].job_post_edit_by_middle_name === '') {
                        jeb = get_jobPost[i].job_post_edit_by_first_name + " " + get_jobPost[i].job_post_edit_by_last_name
                    } else {
                        jeb = get_jobPost[i].job_post_edit_by_first_name + " " + get_jobPost[i].job_post_edit_by_middle_name + " " + get_jobPost[i].job_post_edit_by_last_name
                    }
                }
                if (get_jobPost[i].bill_rate !== null && isNaN(get_jobPost[i].bill_rate) === false) {

                    billRate = get_jobPost[i].bill_rate
                }
                if (get_jobPost[i].blended_pay_rate !== null && isNaN(get_jobPost[i].blended_pay_rate) === false) {

                    blended_pr = get_jobPost[i].blended_pay_rate
                }
                if (get_jobPost[i].regular_pay_rate !== null && isNaN(get_jobPost[i].regular_pay_rate) === false) {

                    regular_pr = get_jobPost[i].regular_pay_rate
                }
                if (get_jobPost[i].at_holiday_rate !== null && isNaN(get_jobPost[i].at_holiday_rate) === false) {
                ...
                data = data +
                    ...
                    "\t" +
                    get_jobPost[i].city + ", " + get_jobPost[i].state + ", " + get_jobPost[i].country +
                    "\t" +
...
function generateExcelManageJobPostData(data, data2) {
    return new Promise(function (resolve, reject) {
        ...
        ws.cell(6, 8)
            ...
            .style(style1);
        ws.cell(6, 9)
            .string("Blended Pay Rate(Traveller $/Hr)")
            .style(style1);
        ws.cell(6, 10)
            .string("Regular Pay Rate(Locals $/Hr)")
            .style(style1);
        ws.cell(6, 11)
            .string("OT/Holiday Rate ($/Hr)")
            .style(style1);
        ws.cell(6, 12)
            .string("Date Of Posting")
            .style(style1);
        ws.cell(6, 13)
            .string("Posted By")
            .style(style1);
...
app.get('/vcsapi/api/generate/excel/assign/manager/:user_id/:name', function (req, res) {
    // if (verifys == "verify") {
    async function apps() {
        try {
            let un = await getAllUser(req.params.user_id);
            ...
            for (i in asmd) {
                if (asmd[i].recruiterData !== undefined) {
                    if (asmd[i].recruiterData.user_middle_name === null || asmd[i].recruiterData.user_middle_name === '') {
                        rn = asmd[i].recruiterData.user_first_name + " " + asmd[i].recruiterData.user_last_name
                    } else {
                        rn = asmd[i].recruiterData.user_first_name + " " + asmd[i].recruiterData.user_middle_name + " " + asmd[i].recruiterData.user_last_name
                    }
                }

                if (asmd[i].onboardData !== undefined) {
                    if (asmd[i].onboardData.user_middle_name === null || asmd[i].onboardData.user_middle_name === '') {
                        obn = asmd[i].onboardData.user_first_name + " " + asmd[i].onboardData.user_last_name
                    } else {
                        obn = asmd[i].onboardData.user_first_name + " " + asmd[i].onboardData.user_middle_name + " " + asmd[i].onboardData.user_last_name
                    }
                }


                if (asmd[i].teamLearData !== undefined) {
                    if (asmd[i].teamLearData.user_middle_name === null || asmd[i].teamLearData.user_middle_name === '') {

                        tln = asmd[i].teamLearData.user_first_name + " " + asmd[i].teamLearData.user_last_name

                    } else {
                        tln = asmd[i].teamLearData.user_first_name + " " + asmd[i].teamLearData.user_middle_name + " " + asmd[i].teamLearData.user_last_name
                    }
                }


                if (asmd[i].managerData !== undefined) {
                    if (asmd[i].managerData.user_middle_name === null || asmd[i].managerData.user_middle_name === '') {
                        mn = asmd[i].managerData.user_first_name + " " + asmd[i].managerData.user_last_name

                    } else {
                        mn = asmd[i].managerData.user_first_name + " " + asmd[i].managerData.user_middle_name + " " + asmd[i].managerData.user_last_name

                    }
                }
                if (asmd[i].user_middle_name === null || asmd[i].user_middle_name === '') {
                    appl_name = asmd[i].user_first_name + " " + asmd[i].user_last_name
                } else {
                    appl_name = asmd[i].user_first_name + " " + asmd[i].user_middle_name + " " + asmd[i].user_last_name
                }
...
function generateExcelAssigmMgrData(data, data2) {
    return new Promise(function (resolve, reject) {
        ...
        wb.write(`/home/ubuntu/vcs/excle_file/assign_mgr_report${data.created_by}.xlsx`, function (err) {
            if (err) resolve("err");
            else resolve(`/home/ubuntu/vcs/excle_file/assign_mgr_report${data.created_by}.xlsx`);

        });
    });
}

app.get('/vcsapi/api/generate/excel/hired/applicant/:user_id/:name', function (req, res) {
    // if (verifys == "verify") {
    async function apps() {
        try {
            let un = await getAllUser(req.params.user_id);
            ...
            for (i in hired) {
                ...
                var shft = ''
                var pp_remark = ''
                if (hired[i].shift_details !== null || hired[i].shift_details !== '') {
                    shft = hired[i].shift_details
                }
                if (hired[i].pay_package_remarks !== null || hired[i].pay_package_remarks !== '') {
                    pp_remark = hired[i].pay_package_remarks
                }
                if (hired[i].per_dieum_wk !== null || hired[i].per_dieum_wk !== '') {
                    pdwk = hired[i].per_dieum_wk
                }
                if (hired[i].ot_starts_after_wk !== null || hired[i].ot_starts_after_wk !== '') {
                    ot_start_wk = hired[i].ot_starts_after_wk
                }
                if (hired[i].contract_duration_wk !== null || hired[i].contract_duration_wk !== '') {
                    contr_dur = hired[i].contract_duration_wk
                }
                if (hired[i].rto !== null || hired[i].rto !== '') {
                    rto = hired[i].rto
                }
                if (hired[i].total_shift_hr !== null || hired[i].total_shift_hr !== '') {
                    tshr = hired[i].total_shift_hr
                }
                if (hired[i].onb_regular_pay_rate !== null || hired[i].onb_regular_pay_rate !== '') {
                    reg_pr = hired[i].onb_regular_pay_rate
                }
                if (hired[i].onb_holiday_pay_rate !== null || hired[i].onb_holiday_pay_rate !== '') {
                    hol_pr = hired[i].onb_holiday_pay_rate
                }
                if (hired[i].onb_ot_pay_rate !== null || hired[i].onb_ot_pay_rate !== "") {
                    ot_pr = hired[i].onb_ot_pay_rate
                }


                if (hired[i].onb_regular_bill_rate !== null || hired[i].onb_regular_bill_rate !== '') {
                    reg_br = hired[i].onb_regular_bill_rate
                }
                if (hired[i].onb_holiday_bill_rate !== null || hired[i].onb_holiday_bill_rate !== '') {
                    hol_br = hired[i].onb_holiday_bill_rate
                }
                if (hired[i].onb_ot_bill_rate !== null || hired[i].onb_ot_bill_rate !== "") {
                    ot_br = hired[i].onb_ot_bill_rate
                }



                if (hired[i].user_middle_name === null || hired[i].user_middle_name === '') {
                    appl_name = hired[i].user_first_name + " " + hired[i].user_last_name
                } else {
                    appl_name = hired[i].user_first_name + " " + hired[i].user_middle_name + " " + hired[i].user_last_name
                }
                if (hired[i].hiring_date !== null || hired[i].hiring_date !== '') {
                    h_date = hired[i].hiring_date
                }
                if (hired[i].proposed_start_date !== null || hired[i].proposed_start_date !== '') {
                    ps_date = hired[i].proposed_start_date
                }
                if (hired[i].proposed_end_date !== null || hired[i].proposed_end_date !== '') {
...
function generateExcelHiredApplicantsData(data, data2) {
    return new Promise(function (resolve, reject) {
        ...
        ws.cell(6, 9)
            ...
            .style(style1);
        ws.cell(6, 10)
            .string("Regular Bill Rate ($/Hr)")
            .style(style1);
        ws.cell(6, 11)
            .string("Holiday Bill Rate ($/Hr)")
            .style(style1);
        ws.cell(6, 12)
            .string("OT Bill Rate ($/Hr)")
            .style(style1);
        ws.cell(6, 13)
            .string("Regular Pay Rate ($/Hr)")
            .style(style1);
        ws.cell(6, 14)
            .string("Holiday Pay Rate ($/Hr)")
            .style(style1);
        ws.cell(6, 15)
            .string("OT Pay Rate ($/Hr)")
            .style(style1);
        ws.cell(6, 16)
            .string("Allowed Per Diem/Week")
            .style(style1);
        ws.cell(6, 17)
            .string("OT Starts After (Hours)")
            .style(style1);
        ws.cell(6, 18)
            .string("Contract Duration (Week)")
            .style(style1);
        ws.cell(6, 19)
            ...
            .style(style1);
        ws.cell(6, 20)
            .string("Shift (Hours/Week)")
            .style(style1);
        ws.cell(6, 21)
            .string("Shift Details")
            .style(style1);
        ws.cell(6, 22)
            .string("Pay Package/Miscellaneous")
            .style(style1);
        ws.cell(6, 23)
        ...
        wb.write(`/home/ubuntu/vcs/excle_file/hired_report${data.created_by}.xlsx`, function (err) {
            if (err) resolve("err");
            else resolve(`/home/ubuntu/vcs/excle_file/hired_report${data.created_by}.xlsx`);

        });
    });
}
app.get('/vcsapi/api/generate/excel/onboarding/report/:user_id/:name', function (req, res) {
    // if (verifys == "verify") {
    async function apps() {
        try {
            let un = await getAllUser(req.params.user_id);
            ...
            for (i in onb) {
                var ddate = ""
                if (onb[i].user_middle_name === null || onb[i].user_middle_name === "") {
                    appl_name = onb[i].user_first_name + " " + onb[i].user_last_name
                } else {
                    appl_name = onb[i].user_first_name + " " + onb[i].user_middle_name + " " + onb[i].user_last_name
                }
...
function generateExcelOnboardingData(data, data2) {
    return new Promise(function (resolve, reject) {
        ...
        wb.write(`/home/ubuntu/vcs/excle_file/onboarding_report${data.created_by}.xlsx`, function (err) {
            if (err) resolve("err");
            else resolve(`/home/ubuntu/vcs/excle_file/onboarding_report${data.created_by}.xlsx`);

        });
    });
}

app.get('/vcsapi/api/generate/excel/job/aplications/:user_id/:name', function (req, res) {
    // if (verifys == "verify") {
    async function apps() {
        try {
            let un = await getAllUser(req.params.user_id);
...
function generateExcelJobApplicationData(data, data2) {
    return new Promise(function (resolve, reject) {
        ...
        ws.cell(6, 7)
            ...
            .style(style1);
        ws.cell(6, 8)
            .string("# of Accepted (Applicant)")
            .style(style1);

        ws.cell(6, 9)
            .string("# of Onboarding")
            .style(style1);
        ws.cell(6, 10)
            .string("# of Hired")
            .style(style1);
        ...
        wb.write(`/home/ubuntu/vcs/excle_file/applicants${data.created_by}.xlsx`, function (err) {
            if (err) resolve("err");
            else resolve(`/home/ubuntu/vcs/excle_file/applicants${data.created_by}.xlsx`);
...
app.get('/vcsapi/api/generate/excel/applicants/aplicant/:user_id/:name', function (req, res) {
    // if (verifys == "verify") {
    async function apps() {
        try {
            let un = await getAllUser(req.params.user_id);
            ...
            for (i in p) {
                ...
                if (p[i].email !== null && p[i].email !== '' && p[i].email !== "null") {
                    email = p[i].email
                }
                if (p[i].phone !== null && p[i].phone !== '' && p[i].phone !== "null") {
                    phone = p[i].phone
                }
                if (p[i].dob !== null && p[i].dob !== '' && p[i].dob !== "null") {
                    b_date = p[i].dob
                }
                if (p[i].current_location !== null && p[i].current_location !== '' && p[i].current_location !== "null") {
                    cur_location = p[i].current_location
                }
                if (p[i].desired_location_1 !== null && p[i].desired_location_1 !== "" && p[i].desired_location_1 !== "null") {
                    if (p[i].desired_location_2 !== null && p[i].desired_location_2 !== "" && p[i].desired_location_2 !== "null") {
                        desired_location = p[i].desired_location_1 + ", " + p[i].desired_location_2
                    }
                    desired_location = p[i].desired_location_1
                }
                if (p[i].ssn_4digit !== null && p[i].ssn_4digit !== '' && p[i].ssn_4digit !== "null") {
                    ssn = p[i].ssn_4digit
                }
                if (p[i].profession_name !== null && p[i].profession_name !== '' && p[i].profession_name !== "null") {
                    prof = p[i].profession_name
                }
                if (p[i].speciality_name !== null && p[i].speciality_name !== '' && p[i].speciality_name !== "null") {
                    spec = p[i].speciality_name
                }
                if (p[i].employement_preference !== null && p[i].employement_preference !== '' && p[i].employement_preference !== "null") {
...
app.post('/vcsapi/api/get/job_no/assignment_id/and/job_id', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/vcsapi/api/get/tbl_week/week_id/not/in/tbl_rec_work_hr', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                ...
                if (getRec.length) {
                    ...
                    if (post2.length > 0) {
                        let post1 = await getRecWorkHourWeekIDS(req.body, getRec[0].recruitee_id);

                        //console.log(post2, post1)
                        if (post1.length > 0) {
                            let r = post2.filter((elem) => !post1.find(({
                                week_id
...
app.post('/vcsapi/api/insert/tbl_rec_work_hr', stuff.verifyToken, stuff.verify, function (req, res) {
    ...
    if (verifys == "verify") {
        async function apps() {
            try {
                ...
                let getREc = await getRecruitee(req.body.user_id);
                if (getREc.length) {
                    let post2 = await getAssignmentDatabyOnboardingID(req.body.assignment_id);
                    if (post2.length > 0) {
                        let diff = parseInt(post2[0].ot_starts_after_wk) - parseInt(req.body.ot_reg_hr)
                        if (diff > 0) {
                            let reg_hr = parseInt(req.body.ot_reg_hr);
                            let ot_hr = 0;
                            let post = await addTblRecWorkHour(req.body, reg_hr, ot_hr, getREc[0].recruitee_id);
                            if (post === "success") {
                                res.json("success");
                            } else {
...
app.get("/vcsapi/get/api/tbl/account_file_data/year/:client", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.get("/vcsapi/get/api/tbl/account_file_data/month/:client/:year", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                //console.log(req.params);
...
app.post("/vcsapi/get/api/payroll_status/approved/assgnmnt_status/workingORclosed/by/applID", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/vcsapi/api/registration/user', function (req, res) {
    async function apps() {
        ...
        if (checkifExists.length) {
            ...
            let insertUser = await adduser(req.body, email.toLowerCase(), mbl_no, hashedPassword);
            if (insertUser === "success") {
                ...
                if (getUser.length > 0) {
                    ...
                    let addrecruitee = await adduserRecruitee(getUser[0].user_id, latestRecruitee1, "no");
                    if (addrecruitee === "success") {
                        let latestRecruitee = await getLatestRecruitee();
                        let recruiteedetails = await adduserRecruiteeDetails(req.body, latestRecruitee[0].recruitee_id);
...
app.post('/vcsapi/api/updateOrInsert/registration/user/:exist/:user_id', function (req, res) {
    async function apps() {
        var email = req.body.email.trim().replace(/\s/g, "");
        var mbl_no = req.body.phone.trim().replace(/\s/g, "");
        ...
        if (checkifExists.length) {
            ...
            if (checkRec.length) {
                if (checkRec[0].registration_status === "no") {
                    let updateUser = await updateuser(req.body, email.toLowerCase(), mbl_no, hashedPassword, checkifExists[0].user_id);
                    if (updateUser === "success") {
                        let updateRec = await updateRecruitee(checkRec[0].recruitee_id, "yes");
                        if (updateRec === "success") {
                            let sendCreds = await sendCredsbyEmail(checkifExists[0], req.body.password);
                            if (sendCreds === "success") {
            ...
            if (req.params.exist === "NO") {
                ...
                if (getUser.length > 0) {
                    if (insertUser === "success") {
                        ...
                        let latestRecruitee1 = await getLatestRecruitee();
                        let addrecruitee = await adduserRecruitee(getUser[0].user_id, latestRecruitee1, "yes");
                        if (addrecruitee === "success") {
                            let latestRecruitee = await getLatestRecruitee();
                            let recruiteedetails = await adduserRecruiteeDetails(req.body, latestRecruitee[0].recruitee_id);
                            if (recruiteedetails === "success") {
                ...
                let updateUser = await updateuser(req.body, email.toLowerCase(), mbl_no, hashedPassword, req.params.user_id);
                if (updateUser === "success") {
                    let role = await getRolebyrole_name();
                    if (role.length) {
                        let get_action_id = await getActionId(role[0].role_id);
                    ...
                    if (rid.length) {
                        ...
                        // if (recruiteedetails === "success") {
                        let sendCreds = await sendCredsbyEmail(userid[0], req.body.password);
                        if (sendCreds === "success") {
...
app.post('/vcsapi/check/email', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
...
app.post('/vcsapi/check/email/register', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            if (req.body.email !== "" || req.body.email === null) {
                var email = req.body.email.trim().replace(/\s/g, "");
...
app.post('/vcsapi/check/email/edit', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            if (req.body.email !== "" || req.body.email === null) {
                var email = req.body.email.trim().replace(/\s/g, "");
...
app.post('/vcsapi/api/login/user', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                var email = req.body.email.trim().replace(/\s/g, "");
                        ...
                        if (ubs.length > 0) {
                            ...
                            if (checkRec.length) {
                                ...
                                if (checkregStatus.length) {
                                    ...
                                    bcryptjs.compare(req.body.password, password, function (err, result) {
                                        if (result == true) {
        ...
        try {
            ...
            if (checkifExists.length) {
                ...
                if (checkRec.length) {
                    ...
                    if (checkregStatus.length) {
                        let sendCreds = await sendforgotPasswordByEmail(num, checkifExists[0].user_first_name, email.toLowerCase());
                        if (sendCreds === "success") {
                            const saltRounds = 10;
                            const hashedPassword = await new Promise((resolve, reject) => {
...
app.post('/vcsapi/api/get/applications', stuff.verifyToken, stuff.verify, function (req, res) {
    ...
    if (verifys == "verify") {
        async function apps() {
            ...
            if (req.body.prefered_location !== "ALL") {
                ...
                str = str + `(!isNull(a.prefered_state) OR isNull(a.prefered_state)) AND  `
            }
            ...
            if (job_id.length > 0) {
                ...
                geta_pplicants = await getApplicantsApplicant(str, job_id_list.join(","));
...
app.get('/vcsapi/api/get/applications/desiredlocation', stuff.verifyToken, stuff.verify, function (req, res) {
    async function apps() {
        if (verifys == "verify") {
...
app.post('/vcsapi/upload/resume/:user', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/vcsapi/upload/registration_api/resume/:exist/:user_id', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                ...
                if (req.params.exist === "NO") {
                    ...
                    if (insertUser === "success") {
                        ...
                        if (getUser.length > 0) {
                            let latestRecruitee1 = await getLatestRecruitee();
                            let addrecruitee = await adduserRecruiteeRegister(getUser[0].user_id, latestRecruitee1, "yes");
                            if (addrecruitee === "success") {
                                let latestRecruitee = await getLatestRecruitee();
                                ...
                                if (recruiteedetails === "success") {
                                    ...
                                    let post = await uploadFile(req.files, latestRecruitee[0].recruitee_id);
                                    // //console.log("post", post)
                    ...
                    if (rid.length) {
                        ...
                        let latestRecruitee1 = await getLatestRecruitee();
                        let addrecruitee = await adduserRecruiteeRegister(req.params.user_id, latestRecruitee1, "yes");
                        if (addrecruitee === "success") {
                            let latestRecruitee = await getLatestRecruitee();
                            //console.log(latestRecruitee)
                            let recruiteedetails = await adduserRecruiteeDetailsRegister(latestRecruitee[0].recruitee_id);
...
app.post('/vcsapi/upload/registration_api/resume/guest/:exist/:user_id/:email', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                //console.log(req.files)
                if (req.params.exist === "NO") {
                    ...
                    if (check.length) {
                        ...
                        if (checkrec.length === 0) {
                            ...
                            let latestRecruitee1 = await getLatestRecruitee();
                            let addrecruitee = await adduserRecruiteeRegister(check[0].user_id, latestRecruitee1, "no");
                            if (addrecruitee === "success") {
                                let latestRecruitee = await getLatestRecruitee();
                                // console.log(latestRecruitee)
                                let recruiteedetails = await adduserRecruiteeDetailsRegister(latestRecruitee[0].recruitee_id);
                            ...
                            let checkrecrResm = await checkRecResume(checkrec[0].recruitee_id);
                            // console.log("ckeckRes--", checkrecrResm)
                            if (checkrecrResm.length > 0) {
                                let upd = await updateResume(req.files, checkrec[0].recruitee_id);
                                // console.log("update", upd)
                                let post = await uploadFile(req.files, checkrec[0].recruitee_id);
                                res.json({
                                    message: post,
                                    // session: req.session.email,
                                    user_details: check[0],
                    ...
                    if (rid.length) {
                        ...
                        let latestRecruitee1 = await getLatestRecruitee();
                        let addrecruitee = await adduserRecruiteeRegister(req.params.user_id, latestRecruitee1, "no");
                        if (addrecruitee === "success") {
                            let latestRecruitee = await getLatestRecruitee();
                            //console.log(latestRecruitee)
                            let recruiteedetails = await adduserRecruiteeDetailsRegister(latestRecruitee[0].recruitee_id);
...
app.post('/vcsapi/check/resume', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/vcsapi/delete/temp/registration', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                let ret = "ERROR";
                        ...
                        if (del_rec === "delete") {
                            ...
                            if (del_rec_det === "delete") {
                                ...
                                if (del_rec_resu === "delete") {
                                    let del_user_access = await deleteUserAccess(req.body.user_id);
        ...
        file.mv("/home/ubuntu/vcs/Uploads/" + filename, function (err) {
            if (err) {
                ...
                let post = {
                    ...
                    upload_date_time: moment(new Date(strTime)).format("MM/DD/YYYY hh:mm:ss A"),
                    expiry_date: ""
                }
                db.query(sql, post, function (err, res) {
                    if (err) {
...
app.post('/vcsapi/get/application/onboarding/user', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
...
app.post('/vcsapi/get/doc_id/other/or/facility', stuff.verifyToken, stuff.verify, function (req, res) {
    async function apps() {
        try {
...
app.post('/vcsapi/api/get/payroll/assignment', stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
...
app.post("/vcsapi/get/assignmentdata/assignment_history/filtered", stuff.verifyToken, stuff.verify, function (req, res) {
    if (verifys == "verify") {
        async function apps() {
            try {
                let str = "";
                ...
                let getREc = await getRecruitee(req.body.user_id);
                if (getREc.length) {
                    let getAss = await getAssignmentHistoryFiltrdData(str, getREc[0].recruitee_id);
                    res.json(getAss);
                } else {
                    res.json("No User found");
                }
